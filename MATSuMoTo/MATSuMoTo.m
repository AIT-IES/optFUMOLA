function [xbest,fbest]=MATSuMoTo(obj,Problem,varargin)
...
warning('off',id) %[PES] turn of warning


timeStart=tic; %record time of algorithm

%% ---------- START INPUT CHECK -------------------
Data = InputCheck(Problem,varargin{:});
Data.Problem='FUMOLA'; %save the name of the problem to the structure
Data.obj = obj; %append FUMOLA object to Data struct
%% ---------- END INPUT CHECK ---------------------
...
xbest=bestPoint;
fbest=bestFval;
%[PES]if matlabpool('size') > 0 %close the matlabpool if necessary
%[PES]    matlabpool close
%[PES]end
