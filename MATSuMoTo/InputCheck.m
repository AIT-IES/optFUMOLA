function Data = InputCheck(Problem,varargin)
...
%% ---------- START INPUT CHECK-------------------
if nargin < 1 %check if input data file present
    error(['You have to supply a problem struct. \n' ...
    'See example files for information how to define problems.'])
else
    Data=Problem; % load problem data
end
...
%[PES]if ~any(strcmp('Parallel Computing Toolbox', {v.Name}))
%[PES]    fprintf(['You do not have MATLAB Parallel Computing Toolbox. \n'...
%[PES]        'Function evaluations will be done in serial. \n'])
%[PES]else
%[PES]    fprintf(['You do have MATLAB Parallel Computing Toolbox. \n '...
%[PES]        'Evaluations will be done in parallel to reduce computation time.\n'])
%[PES]    Data.parallel_eval=1;
%[PES]    if matlabpool('size') == 0 %matlabpool not yet open
%[PES]        matlabpool open
%[PES]    end
%[PES]end
