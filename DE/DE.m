...
I_funevalmax = S_struct.I_funevalmax;
...
I_nfeval      = 0;                    % number of function evaluation
I_funeval = 0;
...
%%% [PES] SPECIAL HANDLING FOR STRUCT OUTPUT FORMAT AND CONSTRAINTS %%%

[i_m, i_n] = size(FM_pop);
feasible_Indices = find(constraints_handle(FM_pop, S_struct));
[j_m, j_n] = size(feasible_Indices);
if j_n > 0 %avoid calling objective function if no point is feasible
    FM_pop_temp = FM_pop(feasible_Indices,:);
    S_val_temp = objective_function(fname,FM_pop_temp,0); %gets objective function values passed as array
	[k_m, k_n] = size(FM_pop_temp);
	I_funeval = I_funeval + k_m;
end

i_count=1;
for i_task=1:i_m
    if any(feasible_Indices==i_task)  %indices where constraints are met
        S_val(i_task).I_nc      = 0; %Number of constraints
        S_val(i_task).FVr_ca    = 0; %Constraint values. 0 means the constraints  are met. Values > 0 measure the distance to a particular constraint.
        S_val(i_task).I_no      = 1; %Number of objectives
        S_val(i_task).FVr_oa(1) = S_val_temp(i_count); %Objective function values
        i_count=i_count+1;
    else %indices where constraints are not met
        S_val(i_task).I_nc      = 1; %Number of constraints
        S_val(i_task).FVr_ca    = Inf; %Constraint values. 0 means the constraints  are met. Values > 0 measure the distance to a particular constraint.
        S_val(i_task).I_no      = 1; %Number of objectives
        S_val(i_task).FVr_oa(1) = Inf; %Objective function values
    end
end
%%% [PES] END SPECIAL HANDLING FOR STRUCT OUTPUT FORMAT %%%

%S_val       = feval(fname,FM_pop,S_struct);
...
I_iter = 1;
while ((I_iter < I_itermax) & (S_bestval.FVr_oa(1) > F_VTR) & (I_funeval < I_funevalmax) )
...
[i_m, i_n] = size(FM_ui);
feasible_Indices = find(constraints_handle(FM_ui, S_struct));
[j_m, j_n] = size(feasible_Indices);
if j_n > 0 %avoid calling objective function if no point is feasible
    FM_pop_temp = FM_ui(feasible_Indices,:);
    S_val_temp = objective_function(fname,FM_pop_temp,I_iter); %gets objective function values passed as array
	[k_m, k_n] = size(FM_pop_temp);
	I_funeval = I_funeval + k_m;
end
...
for i_task=1:i_m
    if any(feasible_Indices==i_task)  %indices where constraints are met
        S_tempval(i_task).I_nc      = 0; %Number of constraints
        S_tempval(i_task).FVr_ca    = 0; %Constraint values. 0 means the constraints  are met. Values > 0 measure the distance to a particular constraint.
        S_tempval(i_task).I_no      = 1; %Number of objectives
        S_tempval(i_task).FVr_oa(1) = S_val_temp(i_count); %Objective function values
        i_count=i_count+1;
    else %indices where constraints are not met
        S_tempval(i_task).I_nc      = 1; %Number of constraints
        S_tempval(i_task).FVr_ca    = Inf; %Constraint values. 0 means the constraints  are met. Values > 0 measure the distance to a particular constraint.
        S_tempval(i_task).I_no      = 1; %Number of objectives
        S_tempval(i_task).FVr_oa(1) = Inf; %Objective function values
    end
end
%%% [PES] END SPECIAL HANDLING FOR STRUCT OUTPUT FORMAT %%%


  %S_tempval = feval(fname,FM_ui,S_struct);   % check cost of competitor
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % [PES] checks if constraints are met
    function const_check = constraints_handle(FM_pop, S_struct)
    % loop over all population members
    for i=1:size(FM_pop,1)
        const_check(i) = 1; % start with constraints are fulfilled
        % loop over all inequality constraints
        for j=1:size(S_struct.b_ineq,1)
            % check if constraints are not fulfilled
            if S_struct.A_ineq(j,:)*FM_pop(i,:)' > S_struct.b_ineq(j,:)
                const_check(i) = 0; % set to constraints are not fulfilled
            end
        end
    end
    end