...
for k = 1:itr
    ...
    if strcmpi(options.ConstrBoundary,'penalize') % EXPERIMENTAL
        if strcmpi(options.Vectorized,'on') % Vectorized fitness function
            state.Score = objective_function( fitnessfcn, state.Population, state.Generation-1) ; %[PES] modified to input iteration counter to objective function
...
%             matlabpool('addattachedfiles',{[pwd '\testfcns\nonlinearconstrdemo.m']}) ;
        ...
        if strcmpi(options.Vectorized,'on')  % Vectorized fitness function
            state.Score(not(state.OutOfBounds)) = ...
                objective_function( fitnessfcn, state.Population(not(state.OutOfBounds),:), state.Generation-1) ; %[PES] modified to input iteration counter to objective function
            %state.Score(not(state.OutOfBounds)) = ...
            %    fitnessfcn(state.Population(not(state.OutOfBounds),:), state.Generation) ;
...
%     tempchk = alpha <= 1.6 ;
    ...
    if options.Verbosity > 0
        if k == 1
            fprintf('\n Iteration || Global Best || Iteration Best\n');
        end
        fprintf('%g %g %g\n',k, state.fGlobalBest(k), minfitness);
