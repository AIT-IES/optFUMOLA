function [BestParticle, BestParticleObj, RunData]=PSwarm(obj, Problem, ...
...
if nargin==2 && nargout <= 2 && isequal(Problem,'defaults')
...
if nargin < 2
  error('pswarm:AtLeastOneInput','PSwarm requests at least two inputs (FUMOLA object and problem definition).');
end

% If parameters are missing just define them as empty
if nargin < 4, Options=[];
    if nargin < 3, InitialPopulation = [];
    end
end
...
%end
% [PES] make FUMOLA object part of problem struct
Problem.obj = obj;
...
Problem.Stats.IterCounterPes=0; %[PES]new iteration counter; no effects on algorithm
