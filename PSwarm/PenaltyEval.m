...
if Problem.mLinear==0
    ...
    FeasibleLinearIdx=find(Problem.A*x(FeasibleBoundsIdx,:)'-repmat(Problem.b,1,length(FeasibleBoundsIdx))<=0); %[PES] removed max(...) to catch ALL points that fulfill constraints
end
...
if(~isempty(FeasibleLinearIdx))
    %try
        %[PES]counts the number of PenaltyEval evaluation (number of actual objective function
        %evaluations can be one ore multiple depending if it is a poll step or
        %particle swarm search step; no effects on algorithm
        Problem.Stats.IterCounterPes=Problem.Stats.IterCounterPes+1;
        PointsEvaltemp = transpose(PointsEval); %[PES] for consistent input handling with optFUMOLAbase
        ObjValuetemp=objective_function(Problem.obj, PointsEvaltemp, Problem.Stats.IterCounterPes-1); %[PES] changed in order to pass iteration counter to objective function
        ObjValue(FeasibleLinearIdx)=transpose(ObjValuetemp); %[PES] for consistent input handling with optFUMOLAbase
